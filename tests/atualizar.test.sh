...
#
#   A) VELHO + REMENDO   o retrato de uma instalação antiga, e por cima o
...
# O 00_tudo.sql "velho": o último antes de o convite ganhar a ficha da agenda.
# É o estado real de quem instalou o sistema naquela semana.
#
# ── POR QUE UM ARQUIVO, E NÃO UM COMMIT ────────────────────────────────────
# A primeira versão lia isto com `git show <sha>:supabase/00_tudo.sql`. Parecia
# mais limpo — nada de copiar 200 KB para dentro do repositório — e quebrou na
# primeira vez que o projeto mudou de casa: o sha era da história do
# repositório `pradotec`, onde o AgendaPro morava numa subpasta. Aqui ele não
# existe, e o teste morreu sem chegar a testar nada.
#
# Quebraria de novo num clone raso, que é o que toda CI faz por padrão: sem
# a história antiga, `git show` não acha o commit por mais que ele exista lá
# no servidor.
#
# Uma base de comparação precisa ser um DADO do teste, não um ponteiro para
# fora dele. Este arquivo é um retrato congelado, e é para ficar congelado:
# ele não representa "o que havia ontem", representa o banco de um cliente
# que instalou naquela semana e não colou mais nada desde então.
#
# Não é para virar "o mais recente": um remendo que só é testado contra ontem
# não testa nada. Só sobe quando o último cliente de verdade passar deste
# ponto — não antes.
# `BASE_VELHA`, e não `BASE`: o tudo.sh exporta `BASE` como a URL do servidor
# estático, e duas coisas diferentes com o mesmo nome numa suíte que roda tudo
# no mesmo shell é armadilha à espera de quem vier depois.
BASE_VELHA="${BASE_ANTIGA:-$AQUI/base-antiga.sql}"
...
[ -f "$BASE_VELHA" ] || { echo "✗ não achei a base antiga em $BASE_VELHA"; exit 1; }

# ── A) o caminho de quem atualiza ─────────────────────────────────────────
nascer atu_velho
psql -q -v ON_ERROR_STOP=1 -d atu_velho -f "$BASE_VELHA" >/dev/null 2>&1 \
  || reprovar "a base antiga (tests/base-antiga.sql) não instala mais"
...
nascer atu_linha
psql -q -v ON_ERROR_STOP=1 -d atu_linha -f "$BASE_VELHA" >/dev/null 2>&1
python3 -c "
import sys
...
nascer atu_sujo
psql -q -v ON_ERROR_STOP=1 -d atu_sujo -f "$BASE_VELHA" >/dev/null 2>&1
